...
class Cubature_rules{
 public:
  ...
  template<typename IntegT, int warp_size> 
    cuhreResult<double> apply_cubature_integration_rules(sycl::queue& q,
    IntegT* integrand, double* lows, double* highs, double& epsrel , double& epsabs, const  Sub_regions<ndim>& subregions, Region_estimates<ndim>& estimates, Region_characteristics<ndim>& chars, bool compute_error = false);
    ...
    */
    ...
    q.submit([&](auto &cgh) {
        cgh.parallel_for(sycl::range<1>(feval), [=](sycl::id<1> feval_id){
            ...
            int posCnt = params._gpuGenPermVarStart[tid + 1] - params._gpuGenPermVarStart[tid];
...
template<typename IntegT, int warp_size>
cuhreResult<double>
Cubature_rules<ndim>::apply_cubature_integration_rules(sycl::queue& q,
    IntegT* integrand, double* lows, double* highs, double& epsrel , double& epsabs, const  Sub_regions<ndim>& subregions, Region_estimates<ndim>& estimates, Region_characteristics<ndim>& chars, bool compute_error){
 ...
 res.estimate = quad::reduction<double>(q, estimates.integral_estimates, subregions.size);
 res.errorest = compute_error ?  quad::reduction<double>(q, estimates.error_estimates, subregions.size) : std::numeric_limits<double>::infinity();
 return res;
}
