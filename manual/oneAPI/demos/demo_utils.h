...
{
  std::cout
    << "alg, id, ndim, warp_size, epsrel, epsabs, value, estimate, errorest, regions, finished_regions, "
       "time, status\n";
}

template<typename F, size_t ndim, int warp_size = 32>
  bool
  time_and_call_pagani(sycl::queue& q, std::string alg_id, std::string integ_id, F& integrand, double true_val, double* lows, double* highs, double epsrel, double epsabs){
      
      bool success = false;
      using Milliseconds = std::chrono::duration<double, std::chrono::milliseconds::period>;
      size_t divs_per_dim = -1;
      ...
      for(int run = 0; run < 11; ++run){
          ...
          auto  res = workspace.template integrate<F, warp_size>(q, integrand, lows, highs, regions, epsrel, epsabs);
          Milliseconds dt = std::chrono::high_resolution_clock::now() - t0;
          std::cout.precision(17);
		  if(run != 0)
			std::cout << std::fixed << std::scientific << alg_id << ","
            << integ_id << ","
            << ndim << ","
            << warp_size << ","
            << epsrel << "," 
            << epsabs << ","
            << std::scientific << true_val << "," 
